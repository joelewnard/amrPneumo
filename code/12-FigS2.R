...
# Kristin Andrejko
# Updated 06-07-21
# This produces Figure S2 with and without secular trends per Lmicorbe editor comment 
...
diff_namesNS <- array(c(outPenNs, outMacNs, outSxtNs,outTetNs,outCephNs), 
                      dim = c(1e4, 171, 10))
diff_namesNS_Secular <- array(c(outPenNs2, outMacNs2, outSxtNs2,outTetNs2,outCephNs2), 
                      dim = c(1e4, 171, 10)) #NEW INCLUDING SECULAR TREND

diff_namesRES <- array(c(outPenRes, outMacRes, outSxtRes, outTetRes, outCephRes), 
                       dim = c(1e4, 171, 10))
diff_namesRES_Secular <- array(c(outPenRes2, outMacRes2, outSxtRes2,outTetRes2,outCephRes2), 
                              dim = c(1e4, 171, 10)) #NEW INCLUDING SECULAR TREND
...
pdf("/Users/kristinandrejko/Box/01-Research/02-SP-AMR/LancetMicrobe-Revision/PneumoAMR-LMicrobe/output/plots/figS2.pdf", width=5, height=4.5) #OLD 
...
for (i in 1:5){ #plot names 
  ...
  # text('10 year difference',x=9.5,adj=1,cex=0.65,y=0.95,xpd=T) #OLD
  # text(round.fn(diff_namesNS[,,i][,101]-diff_namesNS[,,i][,1]),x=9.5,adj=1,cex=0.65,y=0.85,xpd=T) #OLD
  
  text('10 year difference',x=10,adj=1,cex=0.65,y=1,xpd=T) #NEW
  text(paste("Without secular trend:\n",(round.fn(diff_namesNS[,,i][,101]-diff_namesNS[,,i][,1]))),x=10,adj=1,cex=0.65,y=yValues[i],xpd=T) #NEW
  text(paste("With secular trend:\n",(round.fn(diff_namesNS_Secular[,,i][,101]-diff_namesNS_Secular[,,i][,1]))),x=10,adj=1,cex=0.65,y=yValues2[i],xpd=T) #NEW
  ...
  text('10 year difference',x=10,adj=1,cex=0.65,y=1,xpd=T) #NEW
  text(paste("Without secular trend:\n",(round.fn(diff_namesRES[,,i][,101]-diff_namesRES[,,i][,1]))),x=10,adj=1,cex=0.65,y=yValues[i],xpd=T) #NEW
  text(paste("With secular trend:\n",(round.fn(diff_namesRES_Secular[,,i][,101]-diff_namesRES_Secular[,,i][,1]))),x=10,adj=1,cex=0.65,y=yValues2[i],xpd=T) #NEW
